program main
  ...
  double precision, parameter :: XX=0.5d0
...
contains
  ...
  subroutine output
    integer :: i
    character(99) :: name
    write(name, "('data/L', i4.4, '_seed', i4.4, '_p', i3.3, '.spin')") L, seed, p
    ...
    write(name, "('data/L', i4.4, '_seed', i4.4, '_p', i3.3, '.hist')") L, seed, p
    open(10, file=trim(name), action="write")
    do i = 0, N
       if (o(i) == 1) then
          write(10, *) i, f(i)
  ...
  subroutine one_mc
    ...
    do mmc = 0, N - 1
       ...
       call calc_E(E2)
       df = f(E) - f(E2)
       if (df > 0.d0 .or. ur(mmc*2+1) < exp(df)) then
          E = E2
       else
          spin0(i) = 1 - spin0(i)
       end if
